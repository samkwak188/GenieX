...
// One text content part, or none for an assistant turn that carries only tool
// calls.
func textContents(text string) []geniex_sdk.VlmContent {
	if text == "" {
		return nil
	}
	...
	for _, msg := range param.Messages {
		if len(msg.GetToolCalls()) > 0 {
			...
			messages = append(messages, geniex_sdk.VlmChatMessage{
				...
				Contents:  textContents(contentText(msg)),
				ToolCalls: calls,
			})
			continue
		}
